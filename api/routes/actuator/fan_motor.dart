...
Future<Response> doPost(Request request) async {
  ...
  // Estrarre 'speed' dalla richiesta e verificare che sia un intero
  if (data.containsKey('speed') && data['speed'] is int) {
    speed = data['speed'] as int;
  } else {
    return Response.json(
      statusCode: 400,
      body: {'error': 'Invalid speed value'},
    );
  }
  ...
  return Response.json(body: {'status': 'Motor speed successfully forwarded'});
}
...
Future<void> connectToMQTTBroker() async {
  ...
  if (mqttClient.connectionStatus!.state == MqttConnectionState.connected) {
    ...
    await publishAtTopic('fan_motor_mia_api', speed);
  } else {
    print(
...
Future<void> publishAtTopic(String topic, int speed) async {
  // Creazione dell'oggetto JSON
  Map<String, dynamic> data = {'speed': speed};
